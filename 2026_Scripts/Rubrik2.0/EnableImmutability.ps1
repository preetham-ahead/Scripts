...
foreach ($vault in $NonProdVaults) {
    ...
    $action = "None"
    if ($vault.ImmutabilityState -ne "Enabled") {
        ...
        if (-not $DryRun) {
            $body = @{
                properties = @{
                    securitySettings = @{
                        immutabilitySettings = @{ state = "Enabled" }
                    }
                }
    ...
    $Step1Results += [PSCustomObject]@{
        ...
        NewImmutability   = if ($action -eq "None") {$vault.ImmutabilityState} else {"Enabled"}
        Action            = $action
    }
}
